...
**Current branch:** `test` is the integration branch and holds **v0.54.0**, awaiting promotion to `main` (which is at v0.53.0). The promotion is a `test` → `main` PR and needs the owner's explicit go-ahead; expect conflicts in `AGENTS.md`, `CHANGELOG.md` and `ROADMAP.md` only, because `main` carries three docs commits `test` lacks (the README hero captures and the v0.53.0 status refresh, PRs #147/#148) — the earlier dry-run found no source conflicts. Re-check drift before each push (see the branch note in the State paragraph above).
