...
## 1.1 Current project status (as of Sep 22, 2026)
...
- **v0.65.0** — the funnel, the companion and the installable app (2026-09-22, cut on `test`): the Create Trip funnel and ticket, the M5 AI companion, the PWA pair (installability, offline reading, durable sync), the map's honesty pass (the fit frames the drawing; slot search; the P3 sweep), the security hardening batch, one icon vocabulary across every surface, the light-theme AA/palette/ladder cleanup, and `npm run check:migrations` as a release step (its first run found `trips.stay_style` unapplied).
...
- **v0.64.0 — PR #270 (`promote/v0.64.0` → `main`).** 37 commits on `test`: decision comments + settle-up reminder (#268), creator-owned share covers with automatic backfill of third-party images (#261), onboarding A11y fixes at phone width (#269), Explore filters URL-sync, orphaned store ref cleanup, remote-edit banner Board reach, Timeline collapse motion gliding, Plan Bench tablet labels, toast/dock clearance, cited creator-market research. Pending migration: `20260920_decision_comments.sql` (`decisions.comments` column). Changelog coverage audit complete (§2.6b). |
...
- **M5 — AI companion** (tracked as **#236**; its original pair #22 → #20 both closed as audit findings, so the fix went unqueued until M5 shipped it — the LLM endpoint, Jev fast path and offline fallback are built (in `[Unreleased]`)): user-configurable OpenAI-compatible LLM endpoint (`src/lib/aiProvider.ts`), real answers with the deterministic `lib/ai.ts` router kept as offline fallback + an "(LLM)/(offline)" badge
