...
## 1.1 Current project status (as of Sep 11, 2026)

**Version:** v0.51.0 — the timeline restructure + motion + drag release. The 1,500-line `TimelineTab.tsx` monolith is split into modules (`timeline/DaySection|TravelPanel|DaySpark|MoveStopModal|useTimelineMode`, restructure Phases 1+3: one-open-day accordion, day summary rows); motion is governed by a token system (`docs/MOTION-TOKENS.md`, AGENTS rule 10) with the PillNav glider generalized to every pill-style toggle and one frosted-glass recipe for dropdowns/calendar/location; drag-reorder runs entirely on pointer events (free unfenced carry, signed velocity warp, 90ms calm timer, drop zones reading the carried card's CENTRE against stable layout — `insertionIndexFor`/`glideOffsetPx`, both pure and test-pinned; goo/metaball morphing excluded); the day planner gains Optimise (greedy NN + 2-opt, road polylines from corrected legs, asymmetric detours, Google Directions link); Plan-a-trip prefills a live rough-bill budget with toggleable quick chips. On top of v0.50.2's shell topbar removal, v0.50.1's launch-flash fix and v0.50.0's trip-edit persistence fix (every Keep through the impact-preview flow previously rebuilt the day grid from the pre-edit plan and discarded the reorder/delete/move while still toasting "Change saved"; `updateTrip` now reconciles only on actual date changes, pinned by two regression tests that fail on the old code), which sit on v0.49.0's backlog-closing release and the Board-first editing run (in-place board add/edit/delete, Board moved before Timeline, three-part realtime echo-suppression). Promoted to `main` as the v0.51.0 release.

**State:** Stabilization complete, UI audit all 32 findings fixed; the Corridor Concierge suggestion-engine brainstorm is FULLY shipped (Horizons 1–3, 16/16 incl. asymmetry, hours scoring, fuel corridors, trip DNA) — see ROADMAP's 🧭 table. v0.47.0's soft-delete backend is applied live (probe-verified: `trips.deleted_at` exists on the production project; the `get_trashed_trips` RPC is present with authenticated-only EXECUTE — the anon call returns `42501 permission denied`, not PGRST202). Branch model stays two-branch: `main` (production, Vercel) and `test` (integration). `npm run verify` gate: tsc clean + **699 tests** (78 files) + production build.
...
**Current branch:** `test` is the integration branch; as of the v0.51.0 cut it holds the timeline restructure + motion system + bencho drag batch (PRs #100/#101), promoted to `main` as the v0.51.0 release. Re-check drift before each push (see the branch note in the State paragraph above).
