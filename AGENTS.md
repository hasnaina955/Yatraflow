...
**Current branch:** `test` — the integration branch, in sync with `origin/test` at `468fb18` (2026-09-20, the merge of PR #260). `main` sits at `647faf1` (`origin/main`, PR #259) and is now fully **contained in** `test`: `git rev-list --count origin/test..origin/main` returns 0, which is the healthy post-promotion shape. The reverse count is what matters after a promotion — `git rev-list --count origin/main..origin/test` was 3 on 2026-09-18 (the paywall fix), 7 in the other direction mid-window (the promotion merge commits `test` had not yet pulled back), and is 12 now; a non-zero exit from `git merge-base --is-ancestor` alone does not tell you which side is ahead. What `test` carries and production does not (checked 2026-09-20): the public-paywall-RPC fix (`1d4467f`, merged as `0a80197`/PR #256 — the locked-day path, still unpromoted), the crawl surface (`public/robots.txt` + `api/sitemap.js` + its `vercel.json` rewrite, PR #258), the design-system baseline re-keyed off line numbers (`e05c97f`, PR #264), a calmer map rail (`bcf1398`, PR #260), and `.verdent/` scratch untracked (`afb02c0`, PR #263). `package.json` still reads **0.61.0** — none of that is released. **v0.59.0** was the corridor-measurement release, promoted 2026-09-17 (**PR #247**, `main` at `b4f0a18`); **v0.58.0** was the share-card release, promoted 2026-09-17 (**PR #245**, `main` at `cf1c289`). The v0.59.0 work is corridor-wide road measurement, day-scoped measurement, and public addresses that preserve trip-specific previews with navigation that works in any tab. **v0.57.0** was promoted on 2026-09-17 (**PR #244**, `main` at `43c839f`); **v0.56.0** was the settings-integrity release (promoted 2026-09-16, **PR #223**, `main` at `f314b25`). The v0.58.0 work is the share card (`public/og-default.png` plus the `api/i.js` fallback and the shell's own `og:image`) and the Wikimedia cover sizing in `lib/tripThumb.ts`. The v0.57.0 work is the itinerary preview endpoint (`api/i.js` plus the `/i/:id` rewrite that makes a shared link preview as a card), the one-step trip-JSON import, per-route browser-tab titles, the offline companion's routing fixes, and permanent user deletion (**PR #225** — its `admin_delete_user` RPC stays migration-gated on `20260916_admin_delete_user.sql`). The v0.56.0 cycle's own PRs: **#219** (the Settings tab split + propagation fixes, #213 Phases 1–4), **#220** (Create↔Settings parity + numeric defaults, Phases 5+6) and **#221** (the audit docs + ROADMAP queue refresh).
...
- **Unreleased work sits on `test` with no version.** The public-paywall-RPC fix and the crawl surface both live under `[Unreleased]` in `CHANGELOG.md`, so promoting them means a release cut (version bump, entry consolidation — §2.6b) and then its own `test → main` PR under §2.1/§2.8. Two scoped items from v0.60.0 remain deliberately unexecuted in ROADMAP's Idea bank: `overdrive` on its four authored surfaces (**I-18**, now filed as issue **#255** — its contract requires 2–3 directions to be presented and one picked before any code) and theming `::selection`/`caret-color` (**I-17** — a feel change rather than a defect). **I-17's blocker is gone:** it was waiting on the design-system baseline's line-keyed entries being re-mapped, and PR #264 re-keyed every array on the offender's own text (§4), so it is now a one-file, one-hour change
