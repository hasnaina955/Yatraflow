...
## 1.1 Current project status (as of Sep 13, 2026)

**Version:** v0.53.0 — the design-system audit release. An independent AI audit of all 19 pages/sections, 20 overlay surfaces and 20 native `<select>`s was committed as `docs/UI-PAGE-AUDIT.md` and rewritten into **issue #107**, a root-cause-grouped fix tracker — then five batches (PRs #109–#114) worked it to 96 ticked/annotated boxes: **batch 1** fixed every live light-theme contrast failure (deepened `--ink-amber`/`--ink-ok` tier, dark-foreground `#06251f` swap on the solid-teal fill family, `--color-primary` one notch deeper) plus the a11y/layout root causes (`PayerSelect` id forwarding, `.form-row` wrap, `.pulse-bar` span, `.btn.on-teal`); **batch 2** did the fills (3:1 non-text) and routed all raw durations through the motion tokens with the Board FLIP reading `--motion-slow` via `motionTiming()`; **batch 3** was the mechanical tail (`.eyebrow` + five siblings onto the kicker recipe, `pointer: coarse` hit areas, disabled input states, My Trips/Explore/Overview layout fixes, one `--stagger-step`, POI purple tokenized, Admin table scroll, share-tablist affordances); **batch 4** replaced the OS-rendered `<select>` popups with a real ARIA select-only combobox (`components/Select.tsx`, 14 high-traffic surfaces, node-tested keyboard math in `lib/listbox.ts`; six low-traffic selects stay native) and decided SYS-2 as **delete, not adopt** (unused `--text-*`/`--s-*` scales removed); **batch 5** finished the hue work — the "places to see" lane + viewpoint spine own a **292° magenta-violet** (`--yf-poi-see`, 40° clear of `DAY_COLORS[2]` and 38° of the activity purple they used to collide with), Explore gained its entrance stagger, and the Landing canvas drops pinned-attachment on touch. Plus the #106 day-filter regression fix (single-day map view draws the selected day's whole engine journey again). Sits on v0.52.0's map relief release (Liberty basemap + 2D/Terrain/3D view modes), v0.51.0's timeline restructure + motion + drag release, and the v0.49–v0.50 line. Promoted to `main` as the v0.53.0 release.

**State:** Stabilization complete; the v0.40-era 32-finding UI audit AND the deeper #107 tracker are fixed to the floor (96 boxes ticked/annotated; the remainder is recorded owner decisions — budget palette kept as authored, board touch-tooltips product call, two documented conventions). Device-check flags open on hardware only: 3D map-mode frame rate on a mid-range Android, Liberty label density at phone width, Landing scroll after the attachment change. The Corridor Concierge suggestion-engine brainstorm is FULLY shipped (Horizons 1–3, 16/16) — see ROADMAP's 🧭 table. v0.47.0's soft-delete backend is applied live (probe-verified: `trips.deleted_at` exists on the production project; the `get_trashed_trips` RPC is present with authenticated-only EXECUTE — the anon call returns `42501 permission denied`, not PGRST202). Branch model stays two-branch: `main` (production, Vercel) and `test` (integration). `npm run verify` gate: tsc clean + **726 tests** (80 files) + production build.
...
**Current branch:** `test` is the integration branch; as of the v0.53.0 cut it holds the design-system audit release (audit doc + #107 batches 1–5 + #106 fix, PRs #108–#114), promoted to `main` as the v0.53.0 release. Re-check drift before each push (see the branch note in the State paragraph above).
