...
- **The queue is 14 open issues (2026-09-16, `gh issue list --state open`)** — the "the only open issues" framing this section used to carry is false. Two groups: nine **commercial execution** items filed 2026-09-16 under two milestones — `Launch the loop` (#226 E1 → #228 F7) and `Stage 0 · Instrument` (#229 F1 → #234 F6) — and the five **milestone tracks** (**#236 M5 → #240 M9**), which now exist as issues and not only as roadmap prose. Nothing open is a defect; it is all planned work
