...
**Version:** v0.54.0 — the suggestion-pipeline-honesty release, cut on `test` and promoted to `main` as PR #212. Three faults had been draining the Map tab and the Day Planner, and all three were the same shape: a failure that read as "no results" instead of as a failure. (1) **Detours** were `(leg0 + leg1) − routeTotalKm` — one routing engine's route total subtracted from another's internal legs — charging a **+47 km phantom** to every hit on a 1,400 km corridor (a highway petrol pump read "50 km off"), burning the per-day detour budget and holding back most See & do; they are now the geometric spur against the same polyline the search ran on (#187). (2) **The road was measured twice per map open** (workspace + Map tab), doubling the load on the shared OSRM demo that caused the transient failures, and letting the drawn line contradict the detour math; one owner (`src/lib/tripRoad.ts`) now measures once with one retry, and an all-`estimate` chain counts as **unresolved** rather than passing as a road (#188). (3) **The city anchor layer returned zero cities everywhere** — `searchNearby` rejects `administrative_area_level_3`, so every request 400'd and the swallowed error read as "no towns near any halt"; night halts then anchor on OSM `place=city|town`, the only source with town-grade data on rural corridors, with hamlets dropped and a halt over 120 km from its anchor rejected (#189, directive amended in §5). The Day Planner's meal/fuel cadences came back with them (a load-balanced 350 km day absorbed its own lunch and could never fit a fuel stop), and Create Trip gained the Plan Bench's money motion + route-integrity guardrail (#150). Sits on v0.53.0's design-system audit release and the v0.49–v0.52 line. (The v0.53.0 design-system audit story — batches 1–5, 117/117 boxes closed — is recorded in CHANGELOG and `docs/UI-PAGE-AUDIT.md`.)

**State:** Stabilization complete; the v0.40-era 32-finding UI audit AND the deeper #107 tracker are fixed to the floor (**117/117 boxes closed** — every finding carries a verdict; the owner-decision rows are the budget palette kept as authored, the board touch-tooltips product call, and two documented conventions). Device-check flags open on hardware only: 3D map-mode frame rate on a mid-range Android, Liberty label density at phone width, Landing scroll after the attachment change. The Corridor Concierge suggestion-engine brainstorm is FULLY shipped (Horizons 1–3, 16/16) — see ROADMAP's 🧭 table. v0.47.0's soft-delete backend is applied live (probe-verified: `trips.deleted_at` exists on the production project; the `get_trashed_trips` RPC is present with authenticated-only EXECUTE — the anon call returns `42501 permission denied`, not PGRST202). Branch model stays two-branch: `main` (production, Vercel) and `test` (integration). `npm run verify` gate: tsc clean + **885 tests** (93 files) + production build.
...
**Current branch:** `test` is the integration branch and both lines carry **v0.54.0** — the promotion landed as **PR #212**, merging `test` into `main`, so `main` is no longer at v0.53.0. Its only conflict was `ROADMAP.md`, resolved toward `test`'s roadmap-truth rewrite while keeping the `Current version:` pin that the #117/#147 docs-freshness gate asserts; `main`'s three docs commits (the README hero captures and the v0.53.0 status refresh, PRs #147/#148) rode along, as the dry-run predicted. Re-check drift before each push (see the branch note in the State paragraph above).
