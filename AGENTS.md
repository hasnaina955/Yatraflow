...
## 1.1 Current project status (as of Sep 12, 2026)

**Version:** v0.52.0 — the map relief release. The light basemap moved positron → **Liberty** (one line in `mapcn/map.tsx` `defaultStyles`; every map surface inherits; dark keeps the stock dark style — OpenFreeMap ships no Liberty dark), and the Map tab gained three first-class **view modes** on one shared palette (`docs/FEATURE-REQUEST-MAP-VIEWS.md` + `docs/MAP-MOCKUPS.html` landed with the code): flat 2D, **Terrain** (hillshade over the keyless AWS terrarium DEM, inserted before the first `water`/`waterway*` layer — Liberty's river lines sit above `water`, matching `water` alone buries them), and a pitched **3D hero** (`setTerrain` exaggeration 1.8, camera pitch 70 / bearing 235 — `maxPitch` raised 60 → 75 because MapLibre silently clamps). Segmented switcher (three `aria-pressed` chips in a `role="group"`, none "off"); mode persists globally via a `uiPrefs` string pref (corrupt → 2d); the terrain stack re-applies idempotently after every theme style swap (full reload wipes sources/layers/terrain); DEM credit renders only while a terrain mode is on (source added on demand — never `customAttribution`, per the #23 tripwire). The Board stays hard-2D (`enableMapViewModes` opt-in, MapTab only). Pure half in `lib/mapViewModes.ts`, 19 node tests. On top of v0.51.0's timeline restructure + motion + drag release (1,500-line `TimelineTab.tsx` split into modules; motion tokens (`docs/MOTION-TOKENS.md`, AGENTS rule 10); pointer-event drag with centre-based stable-layout drop zones (`insertionIndexFor`/`glideOffsetPx`); Optimise-day (2-opt + road polylines + Google Directions); CreateTrip smart budget prefill), which sits on v0.50.2's shell topbar removal, v0.50.1's launch-flash fix and v0.50.0's trip-edit persistence fix, v0.49.0's backlog-closing release and the Board-first editing run. Promoted to `main` as the v0.52.0 release.

**State:** Stabilization complete, UI audit all 32 findings fixed; the Corridor Concierge suggestion-engine brainstorm is FULLY shipped (Horizons 1–3, 16/16) — see ROADMAP's 🧭 table. v0.47.0's soft-delete backend is applied live (probe-verified: `trips.deleted_at` exists on the production project; the `get_trashed_trips` RPC is present with authenticated-only EXECUTE — the anon call returns `42501 permission denied`, not PGRST202). Branch model stays two-branch: `main` (production, Vercel) and `test` (integration). `npm run verify` gate: tsc clean + **718 tests** (79 files) + production build.
...
**Current branch:** `test` is the integration branch; as of the v0.52.0 cut it holds the map relief release (docs + feature, PR #103), promoted to `main` as the v0.52.0 release. Re-check drift before each push (see the branch note in the State paragraph above).
