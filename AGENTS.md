...
**Current branch:** `test` is the integration branch and carries **v0.56.0**; the promotion to `main` is this release's own PR (branch `promote/v0.56.0`, cut from `test` at `fda3743`). Landed on `test` this cycle: **PR #219** (the Settings tab split + propagation fixes, #213 Phases 1–4), **PR #220** (Create↔Settings parity + numeric defaults, Phases 5+6), **PR #221** (the audit docs + ROADMAP queue refresh), and **PR #224** (the polyline/routing batch — one chain request per corridor, session leg cache, day-filter measuring only the shown day, Google `intermediates` corridor routing, the Return toggle steering suggestion labels, plus the Codacy coordinate-boundary hardening). `main` holds only the docs and merges it already owns until a promote lands. Re-check drift before each push (see the branch note in the State paragraph above).
