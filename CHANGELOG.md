...
- **Card grids are laid out by the grid, not by the stacked-card rule.** `.card + .card { margin-top: 14px }` — the beat between two cards in a column — also matched grid items, so every card after the first in a row rendered 14px lower *and* 14px shorter than the card beside it: measured on the live shelf at tops 839/853 and heights 477/463, with 26px of space where the grid declared 12. The Explore grid, the creator page, My Trips and the public page's Travel-tips / Warnings row all zero that margin now, and the single-column phone layout — where the same rule produced the only visible symptom, one 26px gap among 12px ones — is corrected by the same rule.
...
- **The share URL no longer scrolls the itinerary page sideways on a phone.** A grid item's automatic minimum size is its min-content, and the nowrap share link inside the sticky sidebar gave that column a 451px floor inside a 362px column — the document scrolled 75px sideways at a 390px viewport, and the `text-overflow: ellipsis` the rule already declared could never apply because the flex item refused to shrink. `min-width: 0` on the two-column children lets the column meet the viewport and the ellipsis do its job; the same link renders on the Share tab.
- **The card heart meets the touch floor on phones.** The save control on a grid card is 34px at every width; it now takes 40px under the 720px breakpoint, matching the floor every other control in that block holds.
