...
### Added
- **Importing a trip JSON is now one step, from My Trips, and it reads both formats.** The only import affordance lived inside an existing trip's Share tab, so bringing in a file meant planning a throwaway trip first just to reach the button. `My Trips` now carries **Import JSON** beside *Plan a new trip*, backed by a shared `ImportTripButton` that both places mount — one handler, so the two entry points cannot drift. `parseTripImport()` (`src/lib/tripImport.ts`) is pure and side-effect free, detects the format, fills the collections a hand-written export omits (`expenses` and `fixedCommitments` are both `.map()`ed by `buildTripCopy`, so a missing key was a crash rather than a cosmetic gap), drops the fields a file must not decide (`inviteCode`, `members`, and `visibility` — a gallery file carries `public` and an import must not arrive public), and refuses with a message that names the actual problem: an empty file, a non-JSON file, a snapshot-link payload, a bare publication row, an export with no days, or a truncated day by number. A gallery file's `publication` block is reported to the user rather than applied — publishing stays a deliberate step in the Share tab.
- **A shared itinerary link now previews as a card instead of a bare URL.** The app routes on the hash, so `/#/pub/<id>` never reached a server: every shared itinerary served the same `/` document, and a preview fetcher (WhatsApp, Slack, Facebook, X) had nothing itinerary-specific to read — in the one channel the product is built around. Links are now `/i/<id>`, a real path that `vercel.json` rewrites to `api/i.js`. That function reads the publication and returns the app shell with its own Open Graph tags injected (`og:title` from the title, `og:description` from the author's tagline or, failing that, stated facts — duration, per-person budget, route), plus a canonical link and a hash-set that runs before the SPA boots, so a human lands on the itinerary with no redirect flash. It degrades in two steps rather than failing: with no Supabase env it serves the shell's own default tags, and if the shell cannot be fetched it serves a minimal tagged document with a redirect. The publication id is shape-checked (`^[A-Za-z0-9_-]{1,64}$`) before it reaches a REST query or an inline script, and the inline script escapes angle brackets, because `encodeURIComponent` leaves an apostrophe alone. `index.html` gains a default tag set for site links; it deliberately ships **no `og:image`** — there is no web-facing raster asset in the repo yet, and a 404 image renders worse than none, so the function emits one only when the publication carries an `https` cover. **Existing `#/pub/<id>` links keep working** — the router still resolves them.
- **The browser tab says where you are.** `index.html` carries one static title, so every route shared it: four open tabs all read "YatraFlow — Plan real trips, together", and a bookmark of one itinerary was indistinguishable from a bookmark of the site. `pageTitle()` (`src/lib/pageTitle.ts`) maps the route to a title, applied in one effect in `App.tsx` on hash change. Routes whose name lives in the store (`/trip/…`, `/pub/…`, `/creator/…`) get a generic title there and are refined by the page that already holds the record — `App` deliberately slices its subscriptions, and reading the trips table just to label a tab would undo that.
...
### Changed
- **`.codacy.yml` excludes `api/i.js` from Codacy's static analysis.** The file builds an HTML document, which is its whole job, and the security patterns read that as 23 violations: 15 "user data flows into the host portion of this manually-constructed HTML" on meta tags whose every interpolation already goes through `esc()` (the taint analysis does not recognise `esc()` as a sanitiser), 7 objecting to a function that returns HTML, and 1 "user-controlled URL" on a fetch built from `process.env.SUPABASE_URL` plus an `ID_RE`-checked id. The one genuine finding that check ever produced — the outbound fetch's origin being derived from the caller-controlled `host` header — was fixed in `75554ad`, and its invariants are now pinned by `tests/share-preview.test.ts`, which is narrower and stronger than the pattern that could not see the sanitiser. Per-pattern control is a Codacy UI setting, not a configuration-file feature, so excluding the path is the narrowest instruction the file can carry. The reasoning is recorded in the file itself.

### Fixed
- **`/i/<id>` no longer builds its outbound request from a request header.** The preview function derived its own origin from `x-forwarded-host` / `host` and then passed it to `fetch()` — so the fetch target was caller-controlled. That makes the route an open proxy (it would fetch an arbitrary URL from the deployment's network, including internal addresses) and worse, the fetched body was stripped of its tags, had ours injected, and was returned as `text/html` **under this domain** — an XSS that no escaping in the file could prevent, because the payload would be the document we chose to fetch. The origin now comes from the platform's own environment (`PUBLIC_ORIGIN` → `VERCEL_PROJECT_PRODUCTION_URL` → `VERCEL_URL` → the production default), which also makes `og:url` the stable link rather than whatever host the crawler sent. `tests/share-preview.test.ts` pins that no `req.headers` access exists, so it cannot come back.
- **A gallery itinerary JSON was rejected by the app's own importer.** Every file in `docs/examples/itineraries/` is `{ trip, publication }` — the v0.55.0 contract — and the import check read `days` off the **outer** object, found `trip` and `publication` instead, and refused a file the repo's two gates had already certified with "That file is not a valid YatraFlow trip export". The inner `trip` was importable all along; it simply omits the tool-managed fields (`id`, `createdAt`, `updatedAt`) and `members`, which the importer assigns. `tests/trip-import.test.ts` now parses **every** shelf file the repo ships, so the two formats cannot drift apart again without CI saying so.
- **An imported trip is no longer named "… (copy)".** `buildTripCopy` labels every copy as one, which is right for *Copy this trip* and wrong for a file the user brought: nothing was copied, and a published import carried that label into the public gallery title — which is how the live `Spiti Valley Circuit — Shimla in, Manali out (copy)` row came to exist. `importTrip()` keeps the plan's own name; the copy path is unchanged.
