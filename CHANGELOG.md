...
**A shared itinerary stops shipping an oversized cover, stops printing its own content twice, and the gallery stops featuring an itinerary it cannot vouch for — or showing the same plan twice.** Two labels stop promising what they cannot do: Fork now says it needs an account, and a public page that will not load no longer names a cause it cannot know.
