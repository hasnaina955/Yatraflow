...
- **A second edit no longer discards a staged change.** The impact preview is not modal — the timeline, board and map stay live while it is open — and every mutation used to clone the committed trip, so a change staged while a preview was open replaced it outright: no toast, no merge, no warning. Mutations now chain onto the open preview, so the sheet reports the combined delta (still measured against the plan as saved), a day rename or ride-start edit made while a preview is open is refused with a clear message instead of racing it, Keep refuses to overwrite a plan that changed underneath the preview, and switching trips drops a pending preview rather than writing one trip's shape into another. Move-to-another-day is staged through the same chain, so the numbers on screen describe what Save writes.
