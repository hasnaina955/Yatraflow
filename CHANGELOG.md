...
- **The mobile nav fits a phone, so no route scrolls sideways.** The signed-out chrome reached 38px past the pill at 390px and 28px past the viewport, and `html { overflow-x: clip }` cut "Sign up free" off mid-label instead of scrolling, so every route carried the defect. The auth pair is the only path to an account and every icon control holds the 40px touch floor, which leaves the wordmark as the one part that can give on phones — its anchor keeps `aria-label="YatraFlow home"`, so the link still names itself. A 360px rung then trims the pill's gutters, the tray's padding, the gaps and the logo mark, bringing the row inside the pill from 320px up.
