...
### Security
- **Every live trip was readable by every signed-in user; tombstone visibility is now pinned to the owner team.** The trip-trash repair of Sep 14 left a bare `deleted_at is null` OR-clause in the `trips read hide trashed` SELECT policy. Postgres permissive policies OR-combine, so that clause let any authenticated user read any *live* trip (private ones included), bypassing the base `trips read` restriction (owner / public visibility / member). Found live by the new integration harness — the "non-member cannot see a private trip" probe failed against the real database, three runs in a row, while catalog-only checks stayed green because the escape-hatch tokens existed in the same qual. The policy now reads `auth.uid() = owner_id OR is_editor(trips.id)`, which still satisfies the tombstone-WRITE requirement the Sep-14 repair existed for. Fix is `supabase/migrations/20260917_pin_trashed_read.sql`.
- **The crew-facing RLS shape is now enforced by tests, not intent.** Two layers landed as part of M6 (#237): `supabase/tests/rls_contract.test.sql` — 37 catalog-level assertions over the management plane (policies, SECURITY DEFINER functions, realtime publication membership) runnable in the dashboard SQL editor — and `scripts/integration/integrationHarness.mjs` — an opt-in (`VITE_RUN_INTEGRATION=1 npm run test:integration`) behavioral suite with two throwaway test users that proves cross-user privacy, self-serve join, the editor gate, recipient-only notifications, public gallery reads and realtime round-trips against the live database, with run-id isolation and a printed teardown ledger.
