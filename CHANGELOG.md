...
### Changed
- **The dinner window follows the sun (#122).** It was a constant 20:00–21:00 in every season at every latitude, so a northern December drive planned as if the light lasted until nine — the code even carried a note that sunset-aware ends would "land with #124" while telling travellers to override it by hand. Dinner is now eaten within half an hour of sunset (never before 18:00, never later than the trip's own window) and the night end is an hour past dinner close, so a winter day ends roughly an hour of road earlier than a summer one on the same route and the change can only ever **tighten** a day, never extend it. The sun time rides the Open-Meteo call the Map tab already makes; beyond the forecast window the fixed window stays, and an explicit party setting still wins when it is the earlier of the two.
