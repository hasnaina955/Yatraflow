...
**Snapshot (2026-09-25, verified against the repo):** **`main` and `test` were level at the v0.67.0 promotion — `main` at `af373cc` (the PR #446 merge), tagged annotated `v0.67.0` on the merge commit, the Android APK workflow running on the tag push. The release closed the audit plan's first two waves: sixteen findings, five migrations, every one of them applied live before the promotion, plus the four PRs that were still open when it was cut (the creator hub's own recovery path, the creator fixture, the README refresh and stale-line sweep, and unpinnable map picks asking for their position instead of being dropped). `[Unreleased]` is live again and no PR is open** — v0.66.0 itself was cut on `test` and promoted to `main` the same day (PR #319, `main` at `244e74e`, tagged annotated on the merge commit), and the promotion before it was v0.65.0: cut on `test` and **promoted to `main` the same day (PR #304, `main` at `3893ccc`, 16:01 UTC)**, tagged (annotated `v0.65.0` on the merge commit; the Android APK workflow ran green on the tag push) — under it the v0.64.0 release (PR #270, `main` at `ee4b2ee`) and the v0.62.0 co-editing release, and before that the money release: a paid unlock behind a server-side paywall, a versioned itinerary format, the M6 access-rule suite, and the public surfaces' own pass — promoted to `main` on 2026-09-18 (**PR #259**, `main` at `647faf1`). **v0.60.0** (the whole-app refinement pass, **PR #257**) sits under it. **v0.59.1** was promoted to `main` on 2026-09-17 (**PR #248**, `main` at `1d1b85f`), on top of v0.59.0 (promoted 2026-09-17, **PR #247**, `main` at `b4f0a18`). v0.59.1 is a routing correction: the corridor span measurement assigned each leg by its position among the *uncached* legs rather than within the span, so a cached leg inside the span handed its geometry to the leg after it and cached that under its neighbour's key — the drawn route stopped early, and every later measurement of that leg was served the wrong road. v0.59.0 measures the road one corridor at a time rather than one leg at a time, scopes day-filter measurements to the day on screen, and keeps copied public itinerary addresses previewable with navigation that works in any tab. It sits on v0.57.0, the sharing-and-honesty release: the itinerary preview endpoint (`/i/<id>` answered by `api/i.js` with the publication's own Open Graph tags, then the hash route), the one-step trip-JSON import in both formats the repo ships, permanent user deletion in the masteradmin console, the offline companion's routing collisions fixed, and per-route browser-tab titles. v0.58.0 gives that preview an image: a 1200×630 branded card backs any publication with no cover of its own, and auto-picked Wikipedia covers now go through Wikimedia's own resize endpoint instead of shipping the 1.3–3.3 MB original. It sits on v0.56.0, the settings-integrity release, which lands the full six-phase settings-wiring audit (issue #213, PRs #219/#220/#221): Trip settings as its own eighth workspace tab, party/vehicle preferences persisted to the trips table, the propagation fixes that make a settings change re-derive every downstream surface, style-vs-budget separation, Create↔Settings parity, and the numeric defaults that stop car numbers being written onto bikes and EVs. That release was promoted on 2026-09-16 (**PR #223**, `main` at `f314b25`) and sits on v0.55.0's gallery-pipeline release. `admin_delete_user` keeps its `20260916_admin_delete_user.sql` migration gate. The verify gate stands at tsc + **2270 tests** (169 files) + build.
Current version: **0.67.0** — cut on `test` on 2026-09-25 (the audit plan's first two waves) and **promoted to `main` the same day (PR #446, `main` at `af373cc`, tagged annotated `v0.67.0` on the merge commit)**; the release where the public wire stops telling a reassuring story: the invite code no longer rides along with an anonymous read of a published itinerary, a viewer who has not bought a priced plan receives no budget breakdown (only the publication's own per-person estimate, which the page always showed), and unpublishing keeps the row, so a buyer keeps the access they paid for while the ledger and funnel freeze. It is also where the pruner that deletes traffic history became an operator action, resolving a decision persists, the create flow can neither mint twin trips nor leave a zombie row behind, an unlock is live without a reload, a staged timeline edit survives the next one, the map measures the road once, and a map pick that cannot be pinned asks for its position instead of being dropped. It also makes the creator hub's own recovery path reachable — a failed ledger read had shown a spinner that never resolved, a retry looked like a dead button, and a failed refresh erased figures already on screen. Five migrations, all applied before the promotion. Under it: v0.66.0 — the creator hub as an operating picture and the app's high-end visual pass: the hub opens on a KPI strip and a recorded-traffic trend (visits/forks/unlocks over 7/30/90 days) drawn from the same derivation the rows beneath read, one row per publication with its funnel and actions, the creator profile moved into a disclosure at the foot of the page — every honesty state kept (an unread ledger never reads as an empty one; an un-recorded window says so rather than printing zeroes) — nested funnel figures in the trend's own hues with a monotone-cubic trend that cannot overshoot and invent traffic, a hover/keyboard readout and a chart-navy KPI band, a second critique pass (no zero that was never read, the stage bar a real drop-off, each row stating its window and each rate its denominator), and the hub's 23 Codacy findings cleared (the gate's type-aware ESLint ruleset is invisible to the repo's own `npm run lint`); map-tab search is route-aware (both surfaces run Search-Along-Route over the trip's polyline, the keyless stack ranks merged hits by distance to the corridor, and hits draw as selectable teal markers that clear with the box); and the high-end visual pass lands (Plus Jakarta Sans replaces Inter, one 1.5 icon stroke via `--icon-stroke`, the pill nav's glider animating its own box instead of a distorting `scaleX`, the landing's display scale/macro-whitespace/Double-Bezel buttons, the public itinerary's soft trays, TripCreated's entry choreography, and the OG share card rendering the app's real font again). Also in it: the create funnel's interface review, one assertive form-error beat app-wide with polite field errors, the moment-after's watch-list planning real fuel halts, crew invites on every channel, and theme-aware `::selection`/caret. Four PRs merged to `test` on 2026-09-24: **#315** (`feat/creator-hub-dashboard`, merge `8830648`), **#316** (`fix/map-search-route-bias`, merge `9138ff2`), **#317** (`fix/og-card-font`, merge `3c97d8e`), and **#318** (`docs/promote-v0.66.0`, the release cut, merge `a2d92d9`). Under it: v0.65.0 — cut on `test` and promoted to `main` on 2026-09-22 (PR #304, `main` at `3893ccc`): the Create funnel and its ticket (templates, readiness, radar, route IQ, seasonality, crew invites), the M5 AI companion (a configurable LLM endpoint with Jev routing and an honest offline fallback), the PWA pair (installability, offline reading, durable sync), the map's honesty pass (the fit frames the drawn route, slot search lands a real stop, the P3 sweep), the security hardening batch (rate limiting, headers, the CSPRNG), one icon vocabulary across every surface, and the design AA/palette/spacing-ladder cleanup. Under that: v0.64.0 (decision comments + settle-up, creator-owned share covers, onboarding a11y — PR #270); the 2026-09-22 queue landed on `test` as 12 reviewable integrations with `npm run verify` green at each merge.
...
- [x] **v0.66.0** — the creator hub becomes an operating picture and the app gets its high-end visual pass: **#315** (`feat/creator-hub-dashboard`, merge `8830648`) turns the hub into a dashboard — a KPI strip, a recorded-traffic trend (visits/forks/unlocks over 7/30/90 days) drawn from the same derivation its rows read, one row per publication with its funnel, the profile into a disclosure — with every honesty state kept (an unread ledger never reads as an empty one; an un-recorded window says so rather than printing zeroes), nested funnel figures in the trend's own hues with a monotone-cubic trend that cannot overshoot and invent traffic, a hover/keyboard readout and a chart-navy KPI band, a second critique pass (no zero that was never read, the stage bar a real drop-off, each row stating its window and each rate its denominator), and the hub's 23 Codacy findings cleared (the gate's type-aware ESLint ruleset is invisible to the repo's own `npm run lint`); **#316** (`fix/map-search-route-bias`, merge `9138ff2`) makes map-tab search route-aware (both surfaces run Search-Along-Route over the trip's polyline, the keyless stack ranks merged hits by distance to the corridor, and hits draw as selectable teal markers that clear with the box); **#317** (`fix/og-card-font`, merge `3c97d8e`) makes the OG share card render the app's real font. The same line carries Plus Jakarta Sans, one 1.5 icon stroke, the pill nav's glider animating its own box, the landing's display scale + Double-Bezel buttons, the public itinerary's soft trays, TripCreated's entry choreography, the create funnel's interface review, one assertive form-error beat app-wide, the moment-after's real fuel halts, crew invites on every channel, and theme-aware `::selection`/caret. Cut on `test` 2026-09-24 at `3c97d8e` with `npm run verify` green (2,135 tests / 154 files, 1 skipped) — **promoted to `main` the same day (PR #319, `main` at `244e74e`)**, tagged annotated `v0.66.0` on the merge commit (the Android APK workflow ran green on the tag push).
...
drop, clears the departing account's entries on sign-out, and treats a conflict
(collaborator change after the offline capture) as last-writer-wins with BOTH sides told.
Collaboration actions (votes, comments) still require the network; map tiles and fonts
stay uncached.
...
- **Idea bank I-17 — the selection and the caret join the theme** — `v0.66.0`. Source: the
