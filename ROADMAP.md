...
**Snapshot (2026-09-17, verified against the repo):** **v0.57.0 is cut on `test` and awaiting promotion, and `main` still carries v0.56.0** — the settings-integrity release, which lands the full six-phase settings-wiring audit (issue #213, PRs #219/#220/#221): Trip settings as its own eighth workspace tab, party/vehicle preferences persisted to the trips table, the propagation fixes that make a settings change re-derive every downstream surface, style-vs-budget separation, Create↔Settings parity, and the numeric defaults that stop car numbers being written onto bikes and EVs. It sits on v0.55.0's gallery-pipeline release and was promoted to `main` on 2026-09-16 (**PR #223**, `main` at `f314b25`). Since that promotion the two lines have diverged further: `main` holds the promotion's merge commits, `test` holds the v0.57.0 commits (the preview endpoint, the one-step trip-JSON import, permanent user deletion, the companion's routing fixes and per-route browser-tab titles), and `admin_delete_user` keeps its `20260916_admin_delete_user.sql` migration gate. The verify gate stands at tsc + **1087 tests** (100 files) + build.
