...
| Toggle glider | thumb `transform` slide — a FLIP: the target box's width/height are **written, never transitioned**, and only the transform animates (translate to the old origin, scaled by `oldWidth/newWidth`, settling at `scaleX(1)`), `--motion-med`, `--ease-glide` | `.pill-glider` (workspace tabs, Plan/Inspect, filters, composer mode) |
