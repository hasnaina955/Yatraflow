...
| [`FEATURE-REQUEST-MAP-VIEWS.md`](FEATURE-REQUEST-MAP-VIEWS.md) | Feature request (explanation): three map view modes — 2D on Liberty (proposed new default), Terrain (hillshade relief), 3D hero (pitched terrain) — with a code-grounded implementation sketch, verified keyless endpoints, acceptance criteria and the open decisions. The *how* under ROADMAP idea-bank rows I-17–I-19. **Not started.** |
