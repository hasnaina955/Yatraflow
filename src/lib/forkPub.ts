 ...
 *  stubs) — UNLESS the viewer has paid for the full plan or is the creator
 *  (`unlocked`): their fork carries every day as a real, editable plan,
 *  matching what the public page showed them. Returns false when the
 *  underlying trip vanished, the viewer must log in first (navigation
 *  already handled), or the rows failed to persist — in which case no
 *  half-saved fork is left behind to vanish on the next reload. */
export async function forkPublication(pub: PublishedItinerary, meId: string | null, onNavigate: (r: string) => void, unlocked = false): Promise<boolean> {
  ...
  const hasLockedDays = !unlocked && src.days.some(d => !pub.freeDayIndexes.includes(d.index))
